...
const char config_mqtt_body[] = body_header "<fieldset class=\"set1\"><legend><span><b>Configurações Gateway</b></span></legend><form method='get' action='mq'><p><b>Servidor</b><br><input id='mh' placeholder=" MQTT_SERVER " value='%s'></p><p><b>Porta </b><br><input id='ml' placeholder=" MQTT_PORT " value='%s'></p><p><input type='hidden' id='mu' placeholder=" MQTT_USER " value='%s'></p><p><input id='mp' type='hidden' placeholder=\"Senha\" ></p><p><input style='diplay:hiddein' id='sc' type='checkbox' %s></p><p><b>Nome do Gateway</b><br><input id='h' placeholder=" Gateway_Name " value=\"%s\"></p><p><input type='hidden' id='mt' placeholder=' ' value='%s'></p><br><button name='save' type='submit' class='button bgrn'>Salvar</button></form></fieldset>" body_footer_config_menu;
